...
int main() {
    ...
    for (i = 0; i < MAX_CLIENTS; i++) clients[i].socket = 0;
    ...
    write_log("Sistem", "Server Started");
    ...
    while (1) {
        ...
        for (i = 0; i < MAX_CLIENTS; i++) {
            ...
            if (FD_ISSET(sd, &readfds)) {
                ...
                if ((valread = read(sd, buffer, 1024)) == 0 || strcmp(buffer, EXIT_CMD) == 0) {
                    write_log("Sistem", "Status: Client Disconnected");
                    close(sd);
                    clients[i].socket = 0;
                } else {
                    if (strncmp(buffer, "LOGIN:", 6) == 0) {
                        char *name = buffer + 6;
                        int dup = 0;
                        for(int j=0; j<MAX_CLIENTS; j++) if(strcmp(clients[j].username, name) == 0) dup=1;
                        if(dup) send(sd, "ERR_DUP", 7, 0);
                        else {
                            strcpy(clients[i].username, name);
                            send(sd, "OK", 2, 0);
                            char welcome[100];
                            sprintf(welcome, "\n---Selamat datang di The Wired, %s", name);
                            broadcast(welcome, sd);
                            write_log("Pengguna", "Status: Connected");
                        }
                    } else if (strncmp(buffer, "ADMIN:", 6) == 0) {
                        if (strcmp(buffer + 6, ADMIN_PASS) == 0) {
                            clients[i].is_admin = 1;
                            send(sd, "OK_ADMIN", 8, 0);
                            write_log("Admin", "Status: Authenticated");
                        }
                    } else {
                        char msg[1100];
                        sprintf(msg, "[%s]: %s", clients[i].username, buffer);
                        broadcast(msg, sd);
                        write_log("Pengguna", msg);
                    }
                }
            }
        }
